...
/*   Updated: 2019/01/29 14:14:36 by eviana           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

// A LIRE : CETTE METHODE NE FONCTIONNE PAS, ON NE SAIT PAS LEQUEL DES ELEMENTS SIMILAIRE EST MAL PLACE
// PLUS RAPIDE MAIS FOIREUX
...
// check si on ne depasse pas du board
int		ft_checkboard1(t_point *tab, int x, int y, int size)
{
	...
	{
		ft_putstr("C : piece fit le board\n"); // TEST TEST TEST
		return (1);
	}
	else
		return (0);
...
// check que la place est libre pour la piece
int		ft_checkboard2(char ***board, t_point *tab, int x, int y)
{
	if (board[y + tab[0].y][x + tab[0].x][0] == '.' &&
			board[y + tab[1].y][x + tab[1].x][0] == '.' &&
			board[y + tab[2].y][x + tab[2].x][0] == '.' &&
			board[y + tab[3].y][x + tab[3].x][0] == '.')
	{	
		ft_putstr("D : la place est libre\n"); // TEST TEST TEST
		return (1);
	}
	else
		return (0);
...
int		ft_checkboard3(char ***board, t_point *tab, int x, int y)
{
	ft_putstr("B : je regarde si la piece a deja ete mise\n");
	if (board[y + tab[0].y][x + tab[0].x][tab[4].x] != '0')
	{
		ft_putstr("B' : la piece a deja ete mise a cet endroit\n");
		return (1);
	}
	else
		ft_putstr("B'' : la piece n'a jamais ete mise a cet endroit\n");
		return (0);
}
	...
	while ((tetri != NULL) && i <= ((size * size) - 3))
	{
		while (i <= ((size * size) - 3))
		{
			...
			if (ft_checkboard1(tab, x, y, size) == 1 &&
					!ft_checkboard3(board, tab, x, y) &&
					ft_checkboard2(board, tab, x, y) == 1)
			{
				ft_putstr("E : je positionne \n"); // TEST TEST TEST
				board[y + tab[0].y][x + tab[0].x][0] = tetri->name;
				board[y + tab[1].y][x + tab[1].x][0] = tetri->name;
				board[y + tab[2].y][x + tab[2].x][0] = tetri->name;
				board[y + tab[3].y][x + tab[3].x][0] = tetri->name;
				board[y + tab[0].y][x + tab[0].x][tab[4].x] = tetri->name;
		...
		{	
			...
			y = i / size;
			x = i % size;
			board[y + tab[0].y][x + tab[0].x][0] = '.';
			board[y + tab[1].y][x + tab[1].x][0] = '.';
			board[y + tab[2].y][x + tab[2].x][0] = '.';
			board[y + tab[3].y][x + tab[3].x][0] = '.';
			//board[y + tab[0].y][x + tab[0].x][tab[4].x] = '0'; // EN FAIT ON VEUT GARDER CETTE MEMOIRE
...
{
	char	***board; // FREE BOARD sur plusieurs niveaux ? 
	int 	x;
	int		y;
	int		z;
	...
	while (y < size)
	{
		...
		{
			z = 1;
			if (!(board[y][x] = (char*)malloc(sizeof(char) * (61))))
				return (NULL);
			while (z < 61)
			{
				board[y][x][z] = '0';
...
char	***ft_tetriplace(t_tlist *tetrilist, int	tetrinb)
{
	...
	while (!ft_checkplacetetri(board, tetrilist) && size < 9) // CONDITION SIZE A SUPPRIMER 
	{
		ft_putstr("___________________________JE TENTE UN SIZE DIFFERENT______________________________\n");
		free(board); // FREE BOARD SUR PLUSIEURS NIVEAUX ??
		size++;
		board = ft_createboard(size);
		if (size < 9) // LIGNE A SUPPRIMER !!!!!!!!! CONDITION D'ARRET
			board = ft_backtrack(tetrilist, board, size);
	}
	return (board);
}
